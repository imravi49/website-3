 ...
 * Date: 2025-10-14
 * Purpose: Ensure Google Reviews widget renders persistently for both mobile & desktop.
 * - Moved <ReviewsWidget /> outside Routes so it is always mounted (script loads reliably).
 * - No logic or styling changes anywhere else.
 * REVERT: Replace this file with the REVERT-BELOW section to restore previous behavior.
 */
    ...
    <div className="min-h-screen relative">
      ...
      <main className="pt-20">
        <Routes>
            ...
            element={
              <>
                ...
                <section className="max-w-6xl mx-auto px-6 py-12">
                  <h2 className="text-3xl font-elegant text-center mb-4">
                    Welcome to Ravi Sharma Photo Films
                  </h2>
                  <p className="text-center max-w-3xl mx-auto mb-6">
                    where we infuse magic into your wedding memories turning
                    them into timeless tales of love and companionship.
                  </p>
                  <div className="text-center">
                    <a
                      href="/portfolio"
                      className="px-6 py-3 rounded-full border border-gold bg-gradient-to-r from-transparent to-transparent hero-glow"
                    >
                      See Our Work
                    </a>
                  </div>
          ...
          <Route
            path="/about"
            element={<About onOpenInquiry={() => setInquiryOpen(true)} />}
          />
          <Route
            path="/contact"
            element={<Contact onOpenInquiry={() => setInquiryOpen(true)} />}
          />
      ...
      <a
        href={`https://wa.me/91${7383826282}`}
        className="fixed left-4 bottom-6 p-3 rounded-full shadow-lg bg-white text-black z-50"
      >
        WA
      </a>
      <button
        onClick={() => setInquiryOpen(true)}
        className="fixed right-4 bottom-6 p-3 rounded-full shadow-lg bg-gold text-black z-50"
      >
        Book
      </button>
    </div>
...
/* REVERT-BELOW: original App.jsx before patch
import React, {useEffect, useState} from 'react'
import { Routes, Route } from 'react-router-dom'
import Navbar from './components/Navbar'
import Hero from './components/Hero'
